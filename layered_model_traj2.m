...
clear;close all;
T=10000;dt=1/1000;N=T/dt;L1=2*pi;rk=1;
...
% %         1i*kk0*lx.*(beta./(kk0.^2)-u(:,i-1)).*psik(:,i-1)*dt+...
% %         1i*kk0*lx./(kk0.^2).*hk.*u(:,i-1)*dt+sigmak.*randn(Kmax,1)*sqrt(dt);
...
gap2=100;
traj=[u(gap2:gap2:end);real(psik(1,gap2:gap2:end));real(psik(2,gap2:gap2:end))];
save traj traj
gap1=1;
traj1=[u(gap1:gap1:end);(psik(:,gap1:gap1:end))];
% save traj1 traj1

sampletrajfull=zeros(1+2*Kmax,T);
sampletrajfull(1,:)=u(:,1/dt:1/dt:end);sampletrajfull(2:2:end,:)=psik(:,1/dt:1/dt:end);
sampletrajfull(3:2:end,:)=conj(psik(:,1/dt:1/dt:end));
save sampletrajfull sampletrajfull
x=x(1:Luse,:);
y=y(1:Luse,:);
...
xy=[xx(:),yy(:)];
 G1fixed = (exp(1i * xy * kk1dfullf*2*pi/L1) .* (-ones(nx*ny,1) * transpose(dY))); 
 G2fixed = (exp(1i * xy * kk1dfullf*2*pi/L1) .* (ones(nx*ny,1)* transpose(dX)));
 

load sampletrajfull
time=20;
data=sampletrajfull(:,time);data(1)=0;
ux=G1fixed*data+sampletrajfull(1,time);
uy=G2fixed*data+sampletrajfull(1,time);
if max(abs(imag(ux(:))))>10^(-6) || max(abs(imag(uy(:))))>10^(-6)
    disp('error, complex velicity')
end
