...
for j=1:MM
    fprintf('rest ensemble is %d\n',MM-j);
psikens1(:,1,j)=initialpsik;uens1(:,1,j)=initialu;
...
xens1(:,:,j)=x;yens1(:,:,j)=y;
psikens1(:,:,j)=ufree(2:2:end,:);uens1(:,:,j)=ufree(1,:);
% disp('.......')   
end
save psikens psikens
save uens uens
save psikens1 psikens1
save uens1 uens
...
save xens xens;save yens yens
save xens1 xens1;save yens1 yens1
fprintf('test case %d\n',testcase);
% get xens.mat yens.mat xens1.mat yens1.mat
return
load xens;load yens;
load xens1;load yens1;
...
% xlim([-pi,2*pi+pi]);ylim([-pi,2*pi+pi])
setgca(16)
end