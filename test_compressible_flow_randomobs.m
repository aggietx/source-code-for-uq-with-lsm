rng(1);
n = 8;24; % model dim
L = 10; % num of obs used
L_all = 20; % num of total obs 
epsilon = 1;
...
for i = 2:NN
    G(1:2:end,1:2:end) = real(exp(1i*(x'*kk(1,:)+y'*kk(2,:))).*(ones(L_all,1) * rk(1,:)));
    G(1:2:end,2:2:end) = real(exp(1i*(x'*kk(1,:)+y'*kk(2,:))).* 1i.*(ones(L_all,1) *rk(1,:)));
    G(2:2:end,1:2:end) = real(exp(1i*(x'*kk(1,:)+y'*kk(2,:))).*(ones(L_all,1) *rk(2,:)));
    G(2:2:end,2:2:end) = real(exp(1i*(x'*kk(1,:)+y'*kk(2,:))).* 1i.*(ones(L_all,1) *rk(2,:)));
    G = G*2;
    A1 = G;
    uI(:,i) = uI(:,i-1) + A1 * uII(:,i-1) * dt + Sgm1 * randn(2*L_all,1) * sqrt(dt); % obs up to current
...
index = 1:L;
G = zeros(L*2,3*n);
Sgm1 = sigmal * eye(2*L);InvBoB = (Sgm1 * Sgm1')^(-1);
for i = 2:NN
    if mod(i,100) == 1
        index = randperm(L_all, L);
    end
    x = uI(index*2-1,i-1);x=x';
    y = uI(index*2,i-1);y=y';
    xnew = uI(index*2-1,i);xnew=xnew';
    ynew = uI(index*2,i);ynew=ynew';
    obs_old = reshape([x',y']',2*L,1);
    obs_new = reshape([xnew',ynew']',2*L,1);
    ...
    G = G*2;
    A1 = G;
    mu = mu0 + (a0 + a1 * mu0) * dt + (R0 * A1') * InvBoB * (obs_new-obs_old - A1 * mu0 * dt);
