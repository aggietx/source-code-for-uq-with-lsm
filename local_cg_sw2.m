...
%%% combine with etkf
gamma=35;localupR=1;fprintf('gamma is %2.2f\n',gamma);
MM=200;fprintf('ensemble size is %d\n',MM);
r=0.0;sigma_obs=0.1;fprintf('sigma obs is %2.2f\n',sigma_obs);
...
%   idmeshdrift=get_local_vdID((2*K_max+1),gamma,xexact(:,i),yexact(:,i),L1,0); 
  idmeshdrift=get_local_vdID_L((2*K_max+1),12,xexact(:,i),yexact(:,i),L1);
...
% idx=randperm(L, 12);idx=[idx,idx+L];id=idx(1:length(idx)/2);
% s=idmeshdrift{j}
% id
% pause()

            Lu=invG1(j,:); allnobs(j,i)=length(idx)/2;
...
allrms(i/gap)=rmsvel;
 ...
 [rmstimelocalfil,rmslocalfil,cclocalfil ]=compute_velerrorGBsw(u_hat(:,gap:gap:end),gamma_mean_trace_local(:,gap:gap:end),rk,kk,L1,Dim_Grid,3);
 errlocalfil=[mean(rmstimelocalfil(3:end));mean(rmslocalfil);mean(cclocalfil)];
...
rmscc(real(gamma_mean_trace(ind(2),3:end)),real(u_hat(ind(2),3:end)),1);

disp('....')
rmscc(real(gamma_mean_trace_local(ind(1),3:end)),real(u_hat(ind(1),3:end)),1);
rmscc(real(gamma_mean_trace_local(ind(2),3:end)),real(u_hat(ind(2),3:end)),1);
