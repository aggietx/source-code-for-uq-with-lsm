%% letkf initilization (not rk when applying fft)should use etkf_gb to generate data first
rng(10)
MM=200;gamma=220;sigma_obs=0.1;nfloe=L;sigma_init=0.1;dimuhat0=dimuhat;kk0=kk;
disp('letkf transformation without rk,gb')
...
covph=zeros(1*dimuhat,1*dimuhat,N/gap);
for i=2:N
%     i
...
if mod(i,gap)==0
    %% analysis prepare
 uhatmesh=invfft2mat*uhatnew;%%%%% transform to phy domain
...
% % % s=V*diag(sqrt(1./diag(D)));a=s*s'-Pla;max(abs(a(:)))
    Wla=Wlatemp+avewla*ones(1,MM);  
ensuhatup(imesh,:)=aveuhat(imesh)*ones(1,MM)+Uhat(imesh,:)*Wla;
aveuhatup(imesh)=aveuhat(imesh)+Uhat(imesh,:)*avewla;

    else
       ensuhatup(imesh,:)=uhatmesh(imesh,:);
       aveuhatup(imesh)=aveuhat(imesh);
    end
end %% end of analysi 

 uhat=fft2mat*ensuhatup(1:dimuhat0,:);%%%% temp transform to F domain
