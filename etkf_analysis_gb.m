...
x_pri=[xexact(:,i-2);yexact(:,i-2)]+(A0+A1*gamma_mean0)*dt;
...
R_pri=gamma_cov0 + (a1 * gamma_cov0 + gamma_cov0 * a1' + b1b1t )*dt;
 u_hatpri1 = mvnrnd(mu_pri(halfindgb),R_pri(halfindgb,halfindgb),MM);u_hatpri1=u_hatpri1.';
u_hatpri=zeros(dimuhat,MM);
u_hatpri(halfindgb,:)=u_hatpri1;u_hatpri(halfindgb1,:)=conj(u_hatpri1);
