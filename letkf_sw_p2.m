%% letkf initilization
rng(10);MM=200;gamma=2.0;sigma_obs=0.1;nfloe=L;sigma_init=0.1;
...
aveuhatup([imesh;imesh+1*dimuhat0;imesh+2*dimuhat0])=aveuhat([imesh;imesh+1*dimuhat0;imesh+2*dimuhat0])+...
  ...
  ub=G*ensuhatup(1:dimuhat0,:);%%%% temp transform to F domain
  vb=G*ensuhatup(1+dimuhat0:2*dimuhat0,:);
  etab=G*ensuhatup(1+dimuhat0*2:3*dimuhat0,:);
  
  u_mean_post=sum(ensuhatup,2)/MM;
    Upost=ensuhatup - u_mean_post * ones(1, MM);
% covph(:,:,i/gap)=Upost*Upost';
covph(:,:,i/gap)=cov(Upost.');
...
% %         else
       ...
       uhat(ind,:)=rmatrix\[ux;vx;etax];
% % %         end
% %         end
    end  
    uhat([1,1+dimuhat0,1+2*dimuhat0],:)=0;
