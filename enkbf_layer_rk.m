...
%% step 1
k1a=-dkrep.*psikens*dt+...
        1i*(kk0*lx.*( (beta./(kk0.^2)/lx^2)*ones(1,MM) -repmat(uens,Kmax,1))).*psikens*dt+...
        (1i*((kk0*lx./(kk0.^2)/lx^2.*hk)*ones(1,MM)).*repmat(uens,Kmax,1)*dt);
k1b=-du*uens*dt-1i*lx*sum((kk0.*hk)*ones(1,MM).*conj(psikens))*dt+...
        -1i*lx*sum(-(kk0.*hkstar)*ones(1,MM).*psikens)*dt;
%      whos k1b
%     pause()
%% step2
psikens=psikens+k1a*dt/2;uens=uens+k1b*dt/2;
k2a=-dkrep.*psikens*dt+...
        1i*(kk0*lx.*( (beta./(kk0.^2)/lx^2)*ones(1,MM) -repmat(uens,Kmax,1))).*psikens*dt+...
        (1i*((kk0*lx./(kk0.^2)/lx^2.*hk)*ones(1,MM)).*repmat(uens,Kmax,1)*dt);
k2b=-du*uens*dt-1i*lx*sum((kk0.*hk)*ones(1,MM).*conj(psikens))*dt+...
        -1i*lx*sum(-(kk0.*hkstar)*ones(1,MM).*psikens)*dt;
%     whos k2b
%     pause()
%% step3
psikens=psikens+k2a*dt/2;uens=uens+k2b*dt/2;
k3a=-dkrep.*psikens*dt+...
        1i*(kk0*lx.*( (beta./(kk0.^2)/lx^2)*ones(1,MM) -repmat(uens,Kmax,1))).*psikens*dt+...
        (1i*((kk0*lx./(kk0.^2)/lx^2.*hk)*ones(1,MM)).*repmat(uens,Kmax,1)*dt);
k3b=-du*uens*dt-1i*lx*sum((kk0.*hk)*ones(1,MM).*conj(psikens))*dt+...
        -1i*lx*sum(-(kk0.*hkstar)*ones(1,MM).*psikens)*dt; 
%         whos k3b
%     pause()
%% step4
psikens=psikens+k3a*dt/2;uens=uens+k3b*dt/2;
k4a=-dkrep.*psikens*dt+...
        1i*(kk0*lx.*( (beta./(kk0.^2)/lx^2)*ones(1,MM) -repmat(uens,Kmax,1))).*psikens*dt+...
        (1i*((kk0*lx./(kk0.^2)/lx^2.*hk)*ones(1,MM)).*repmat(uens,Kmax,1)*dt);
k4b=-du*uens*dt-1i*lx*sum((kk0.*hk)*ones(1,MM).*conj(psikens))*dt+...
        -1i*lx*sum(-(kk0.*hkstar)*ones(1,MM).*psikens)*dt; 
%         whos k4b
%     pause()
...
psikens=psikenstemp+ksa*dt+...
        1/sqrt(2)*sigmakm*(randn(Kmax,MM)+1i*randn(Kmax,MM))*sqrt(dt)+daterm(2:2:end,:);    
uens=uenstemp+ksb*dt+sigmau*randn(1,MM)*sqrt(dt)+daterm(1,:);
psikenstemp=psikens;
uenstemp=uens;
    if mod(i,1/dt)==0
      estu(i*dt)=mean(uens);  
