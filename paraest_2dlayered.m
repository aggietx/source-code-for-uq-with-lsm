...
%%%% the only input is the tracer trajectory generated by layered_model_tra_2d
%%%% iterative parameter estimation for impressible flow with reduced modes
disp('paraest_2dlayered');if exist('p','var');fprintf('p is %d \n',p);end
K_max=Kmax;Nit=1;maxlag=5000;funfit=0;tfit=1;fprintf('lag si %d \n',maxlag);fprintf('total iteration %d \n',Nit);
...
%         disp(i*dt)
 fprintf('rest step is %d\n',N-i);
    ...
    G1 = (exp(1i * x_loc * kk*2*pi/L1) .* (-ones(L,1) * transpose(dY))); % Fourier bases for u
    G2 = (exp(1i * x_loc * kk*2*pi/L1) .* (ones(L,1)* transpose(dX))); % Fourier bases for v
...
estuhat=gamma_mean_trace;

rmscc(estuhat(1,:),psik(1,:),1);
rmscc(real(estuhat(getind(1,0,kk),:)),real(psik(getind(1,0,kk),:)),1);
...
trajestfil=real(gamma_mean_trace([ind1,ind2,ind3],ts*gap:gap:end));
save trajestfil trajestfil
return
% estuhat=gamma_mean_trace_smoother(:,1/dt:1/dt:end);
% uexact=u(:,1/dt:1/dt:end);
% psikexact=psik(:,1/dt:1/dt:end);
