function [dk,omegak,mu,sigmak2]=fit_ou_fun_sw(funfit,K_max,maxlag1,maxlag2,dt,u_hat,kk)
% [dk,omegak,sigmak2,mu]=fit_ou_fun(0,K_max,500,dt,u_hat,kk);
%%%%% gb:funfit=0,gr:funfit=1
%%% shallow water 
% Kmax=K_max;
% Ktol=18;%funfit=0;
...
for ind=1:size(dk,1)    
    ...
    if ind<length(dk)/3+1
    Nf = maxlag1; xft = 0:dt:maxlag1*dt; % number of lags
    maxlag=maxlag1;
    else
     Nf = maxlag2; xft = 0:dt:maxlag2*dt; % number of lags 
      maxlag=maxlag2;
    end
streamf=u_hat(ind,:);
...
acf=autok;
% if funfit
if ind>length(dk)/3 %%%% funfit for gr
